    ...
    readdy::model::top::graph::Vertex* main_vertex)
{
    ...
    for(std::size_t i = 0; i < main_vertex->neighbors().size(); ++i)
    {
        ...
        if(ptype.find("_x") != std::string::npos) { out_vertices[1] = &(*main_vertex->neighbors()[i]); }
        else if(ptype.find("_y") != std::string::npos) { out_vertices[2] = &(*main_vertex->neighbors()[i]); }
        else if(ptype.find("_z") != std::string::npos) { out_vertices[3] = &(*main_vertex->neighbors()[i]); }
	}
	return out_vertices;
}
