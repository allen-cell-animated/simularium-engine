    ...
    std::vector<readdy::model::top::graph::Vertex*>& ignore_list)
{
    ...
    readdy::model::top::graph::Vertex* first_vertex = &*std::find_if(
        ...
        [&first_parent_state, &particles, &ignore_list](readdy::model::top::graph::Vertex& vert) {
    ...
    float angle_force_constant)
{
    ...
    std::string tf = rx.name + "_" + rx.complexes[0].parents[0].name + "_" + rx.complexes[1].parents[0].name + "_" + rx.product;
    ...
    for (auto center : rx.centers) {
        ...
        bond.length = model.agents[center.child_1].collision_radius + model.agents[center.child_2].collision_radius;

        bond.forceConstant = bond_force_constant;
        topologies.configureBondPotential(child1, child2, bond);

        float radius = model.agents[parent1].collision_radius + model.agents[parent2].collision_radius;
...
void construct_fusion_reaction(readdy::Simulation* sim, aics::agentsim::Reaction& rx)
{
    ...
    std::string rx_string = rx.name + ": " + rx.complexes[0].name + "(" + rxr1 + ") + " + rx.complexes[1].name + "(" + rxr2 + ") -> " + tf + "(" + pf1 + "--" + pf2 + ") [self=false]";
    ...
    auto srx_func = [](readdy::model::top::GraphTopology& top) {
        ...
        for (auto center : reaction.centers) {
            ...
            reposition_location += reposition1 ? 2 * top.particleForVertex(child2).pos() - top.particleForVertex(*reactant2[0]).pos() : readdy::Vec3(0, 0, 0);

            reposition_location += reposition2 ? 2 * top.particleForVertex(child1).pos() - top.particleForVertex(*reactant1[0]).pos() : readdy::Vec3(0, 0, 0);
    ...
    aics::agentsim::Reaction& rx)
{
    ...
    auto srx_typing_func = [](readdy::model::top::GraphTopology& top) {
        ...
        auto count_agents =
            [&particles, &vertices](std::unordered_map<std::string, std::size_t>& map,
                std::size_t& total) {
                for (auto vert = vertices.begin(), end = vertices.end(); vert != end; ++vert) {
